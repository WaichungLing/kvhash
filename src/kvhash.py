...
class KVHashCache(Cache):
    ...
    def head_eviction(self, scorer, l_id, h_id, budget):
        ...
        # ADD Pooling
        scorer_pooled = scorer.unsqueeze(0).unsqueeze(0)
        scorer_pooled = F.max_pool1d(
            scorer_pooled, kernel_size=self.kernel_size, padding=self.kernel_size//2, stride=1)
        scorer_pooled = scorer_pooled.squeeze(0).squeeze(0)
